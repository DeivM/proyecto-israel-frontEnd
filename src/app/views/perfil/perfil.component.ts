...
export class PerfilComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.indexParameters.modulo = modules.perfil.name;
  ...
  getDataForm(id: number) {
    this.itemSub = this.httpService.GetDataForm(id, undefined, modules.perfil.name)
      .subscribe(item => {
        if (item.data?.data) {
          this.itemForm.patchValue(item.data?.data);
          this.itemForm.patchValue({
            perEstado:ConvertirIntBool(item.data?.data.perEstado)
        })
  ...
  onSubmit() {
    ...
    this.itemForm.patchValue({
      perEstado:ConvertirBoolInt(this.itemForm.value.perEstado)
  })
    this.itemsSub = this.httpService.PostPut(this.itemForm.value, modules.perfil.name, false, undefined, this.itemForm.value.perId)
      .subscribe(data => {
        this.closeBtnClick();
  ...
  delete(id:number)
  {
    ...
    this.itemsSub = this.httpService.deleteItems( modules.perfil.name,  undefined, itemsDelete)
    .subscribe(data => {
      this.closeBtnClick();
