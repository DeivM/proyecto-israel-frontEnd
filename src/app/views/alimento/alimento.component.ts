...
export class AlimentoComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.itemForm = this.fb.group({
      aliId: [0],
      aliNombre: [null, Validators.required],
      aliDescripcion: [null],
      aliCategoria: [null, Validators.required],
      aliServicio: [null, Validators.required],
      aliCalorias: [null, Validators.required],
      aliUrl: [null],
    });
    ...
    this.indexParameters.modulo = modules.alimento.name;
  ...
  getDataForm(id: number) {
    this.itemSub = this.httpService.GetDataForm(id, undefined, modules.alimento.name)
      .subscribe(item => {
        if (item.data?.data) {
  ...
  onSubmit() {
    ...
    this.itemsSub = this.httpService.PostPut(this.itemForm.value, modules.alimento.name, false, undefined, this.itemForm.value.aliId)
      .subscribe(data => {
        this.closeBtnClick();
  ...
  delete(id:number)
  {
    ...
    this.itemsSub = this.httpService.deleteItems( modules.alimento.name,  undefined, itemsDelete)
    .subscribe(data => {
      this.closeBtnClick();
