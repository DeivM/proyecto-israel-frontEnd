...
export class MedicoComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.itemForm = this.fb.group({
      medId: [0],
      medNombres: [null, Validators.required],
      medApellidos: [null, Validators.required],
      medCedula: [null, Validators.required],
      medDireccion: [null, Validators.required],
      medCorreo: [null, Validators.required],
      medTelefono: [null, Validators.required],
      medSexo: [null, Validators.required],
      medNumCsp: [null, Validators.required],
      medEstado: [true],
    });
    ...
    this.indexParameters.modulo = modules.medico.name;
  ...
  getDataForm(id: number) {
    this.itemSub = this.httpService.GetDataForm(id, undefined, modules.medico.name)
      .subscribe(item => {
        if (item.data?.data) {
          this.itemForm.patchValue(item.data?.data);
          this.itemForm.patchValue({
            medEstado:ConvertirIntBool(item.data?.data.medEstado)
        })
  ...
  onSubmit() {
    ...
    this.itemForm.patchValue({
      medEstado:ConvertirBoolInt(this.itemForm.value.medEstado)
  })
    this.itemsSub = this.httpService.PostPut(this.itemForm.value, modules.medico.name, false, undefined, this.itemForm.value.medId)
      .subscribe(data => {
        this.closeBtnClick();
  ...
  delete(id:number)
  {
    ...
    this.itemsSub = this.httpService.deleteItems( modules.medico.name,  undefined, itemsDelete)
    .subscribe(data => {
      this.closeBtnClick();
