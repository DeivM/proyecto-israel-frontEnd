...
@Component({
  selector: 'app-indiceSeguimiento',
  templateUrl: './indiceSeguimiento.component.html',
  styleUrls: ['./indiceSeguimiento.component.css']
})
export class IndiceSeguimientoComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.itemForm = this.fb.group({
      iseId: [0],
      iseNumero: [null, Validators.required],
    ...
    this.indexParameters.modulo = modules.indiceSeguimiento.name;
  ...
  getDataForm(id: number) {
    this.itemSub = this.httpService.GetDataForm(id, undefined, modules.indiceSeguimiento.name)
      .subscribe(item => {
        if (item.data?.data) {
  ...
  onSubmit() {
    ...
    this.itemsSub = this.httpService.PostPut(this.itemForm.value, modules.indiceSeguimiento.name, false, undefined, this.itemForm.value.iseId)
      .subscribe(data => {
        this.closeBtnClick();
  ...
  delete(id:number)
  {
    ...
    this.itemsSub = this.httpService.deleteItems( modules.indiceSeguimiento.name,  undefined, itemsDelete)
    .subscribe(data => {
      this.closeBtnClick();
